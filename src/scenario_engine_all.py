# src/scenario_engine_all.py
"""
Machine à états pour les 4 scénarios d'interaction QTRobot (Config "all").
...
class ScenarioEngineAll:
    """
    Moteur de scénario. Gère la logique d'états et les transitions pour les 4 scénarios.
    """

    def __init__(self, scenario_id, audio_manager, vest_manager, ui_callback=None):
        """
        :param scenario_id: 1, 2, 3 ou 4
    ...
    # ═══════════════════════════════════════════════════════════════
    #  SCÉNARIO 1 — Le Réconfort (Tristesse → Câlin)
    # ═══════════════════════════════════════════════════════════════
    def _run_scenario_1(self):
        target_emotion = "Sadness"
        target_gesture = "Calin"
        ...
        self.audio.speak(
            ...
            "Tu veux bien me faire un gros câlin pour nous réconforter ?"
        )
        time.sleep(1)
    ...
    # ═══════════════════════════════════════════════════════════════
    #  SCÉNARIO 2 — L'Encouragement Joyeux (Joie → Tape_Attention)
    # ═══════════════════════════════════════════════════════════════
    def _run_scenario_2(self):
        target_emotion = "Happiness"
        target_gesture = "Tape_Attention"
        ...
        self.audio.speak(
            ...
            "Allez, tope-là ! Fais-moi quelques petites tapes amicales sur l'épaule !"
        )
        time.sleep(1)
    ...
    # ═══════════════════════════════════════════════════════════════
    #  SCÉNARIO 3 — Le Rêve (Veille → Chatouilles)
    # ═══════════════════════════════════════════════════════════════
    def _run_scenario_3(self):
        target_gesture = "Chatouilles"
        ...
        # Le robot attend de détecter les chatouilles
        self._wait_for_gesture(target_gesture)
        if not self.running:
            return
        ...
        # 2. Le pincement/chatouille a été vu, on rallume l'écran !
        self.audio.screen_on()
        ...
        self.audio.speak(
            ...
            "Bonjour toi, tes chatouilles m'ont réveillé, tu es prêt à jouer ?"
        )

    # ═══════════════════════════════════════════════════════════════
    #  SCÉNARIO 4 — La Tristesse (Tristesse robot → Caresse_Reconfortante)
    # ═══════════════════════════════════════════════════════════════
    def _run_scenario_4(self):
        target_gesture = "Caresse_Reconfortante"
        target_emotion = "Happiness"

        # ── DÉBUT ──
        self.state = ScenarioState.DEBUT
        self._update_ui("😢 DÉBUT — Le robot adopte un profil vulnérable et triste")
        
        # Robot triste : tête un peu baissée, regarde le sol
        self.audio.move_head(0, 30)  
        self.audio.play_emotion("QT/sad")
        time.sleep(1)
        self.audio.speak("Je me sens un peu triste aujourd'hui...")
        time.sleep(1)

        # ── EXPLORATION / ACTION ──
        self.state = ScenarioState.ACTION
        self._update_ui(
            "🔍 ATTENTE — L'enfant est incité à réconforter librement... "
            f"En attente de '{target_gesture}' ({GESTURE_STREAK_REQUIRED}× consécutives)"
        )
        
        # Le robot attend la caresse réconfortante
        self._wait_for_gesture(target_gesture)
        if not self.running:
            return

        # ── RÉACTION / DEMANDE DE SOURIRE ──
        self.state = ScenarioState.REACTION
        self._update_ui("🌟 RÉACTION — Le robot se redresse et demande un sourire")
        
        # Le robot lève la tête et commence à aller mieux
        ...
        self.audio.speak(
            ...
            "Pour me remonter le moral à fond, un petit sourire de ta part me ferait très plaisir !"
        )
        ...
        # ── CONCLUSION ──
        self.state = ScenarioState.CONCLUSION
        self._update_ui("🎉 CONCLUSION — Le robot est pleinement réconforté")
        self.audio.play_emotion("QT/happy")
        self.audio.play_gesture("QT/kiss")
        time.sleep(1)
        self.audio.speak("Wahou, ton sourire est super ! Je me sens complètement bien maintenant, merci beaucoup pour ta gentillesse !")
